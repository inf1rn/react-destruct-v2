...
import { clearFilters, setCurrentFilters, setFiltersKeyword, setPaginationPage, setFiltersDateAt, setFiltersDateTo } from '../../../store/parsers/logs/vk/groups/reducer'
import { parsersLogsVkGroupsSelectors } from '../../../store/parsers/logs/vk/groups/selectors'
...
export const ParsersVkGroups = () => {
  const [getExcel] = parsersAPI.useLazyGetParserLogsExcelQuery()

  const filters = useSelector(parsersLogsVkGroupsSelectors.filters)
  const currentFilters = useSelector(parsersLogsVkGroupsSelectors.currentFilters)
  const pagination = useSelector(parsersLogsVkGroupsSelectors.pagination)
  ...
  const [startParser, {isSuccess: isSuccessStartingParser}] = parsersAPI.useStartVkGroupsParserMutation()
