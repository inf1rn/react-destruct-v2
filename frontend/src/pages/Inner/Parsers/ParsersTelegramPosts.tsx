...
import { setFiltersDateAt, setCurrentFilters, setFiltersKeyword, setPaginationPage, clearFilters, setFiltersDateTo } from '../../../store/parsers/logs/telegram/posts/reducer'
import { parsersLogsTelegramPostsSelectors } from '../../../store/parsers/logs/telegram/posts/selectors'
...
export const ParsersTelegramPosts = () => {
  const [getExcel] = parsersAPI.useLazyGetParserLogsExcelQuery()

  const filters = useSelector(parsersLogsTelegramPostsSelectors.filters)
  const currentFilters = useSelector(parsersLogsTelegramPostsSelectors.currentFilters)
  const pagination = useSelector(parsersLogsTelegramPostsSelectors.pagination)
  ...
  const [startParser, { isSuccess: isSuccessStartingParser },] = parsersAPI.useStartTelegramPostsParserMutation()
